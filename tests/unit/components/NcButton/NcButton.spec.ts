...
describe('NcButton', () => {
	...
	describe('toggle button behavior', () => {
		...
		it('has no aria-pressed when pressed is null', () => {
			const wrapper = shallowMount(NcButton, { props: { ariaLabel: 'button', pressed: null } })
			const button = wrapper.find('button')
			expect(button.attributes('aria-pressed')).not.toBeDefined()
		})

		it('has no aria-pressed for a link', () => {
			const wrapper = shallowMount(NcButton, { props: { ariaLabel: 'button', pressed: true, href: 'http://example.com' } })
