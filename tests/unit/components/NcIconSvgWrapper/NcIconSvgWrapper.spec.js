 ...
 */
function mountNcIconSvgWrapper(props) {
	return shallowMount(NcIconSvgWrapper, { props })
}
...
describe('NcIconSvgWrapper', () => {
	...
	it('should render SVG in a span with aria-hidden when no name is provided', () => {
		...
		expect(wrapper.find('span').attributes('aria-label')).not.toBeDefined()
	})
	...
	it('should render SVG in a span with aria-label when name is provided', () => {
		...
		expect(wrapper.find('span').attributes('aria-hidden')).not.toBeDefined()
		expect(wrapper.find('span').attributes('aria-label')).toBe('Check')
	})
