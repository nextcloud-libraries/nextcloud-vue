			...
			if (!String(type).startsWith('#')) {
				...
				if (this.useExtendedMarkdown) {
					...
					if (String(type) === 'pre' && children && String(children.type) === 'code') {
						...
						return h('p', { class: 'rich-text--code-block' }, [
							h(type, { ...props, id }, children),
							h(NcRichTextCopyButton, { class: 'rich-text--code-copy-button', contentId: id }),
						])
					}
...
.rich-text--code-block {
	...
	&:hover .rich-text--code-copy-button,
	&:focus-within .rich-text--code-copy-button,
	& .rich-text--code-copy-button:focus {
		opacity: 1;
	}
}
</style>
