...
<template>
	<span
		...
		<NcPopover
			v-else-if="hasMenu"
			:shown.sync="contactsMenuOpenState"
			...
			<NcProfileHoverCard
				...
				:preloaded-user-status="hoverCardPreloadedUserStatus" />
		</NcPopover>
...
export default {
	...
	computed: {
		 ...
		 */
		hoverCardPreloadedUserStatus() {
			if (this.preloadedUserStatus) {
				return this.userStatus
			}
			if (this.userStatus.status || this.userStatus.icon || this.userStatus.message) {
				return this.userStatus
			}
			return undefined
		},
