...
<template>
	<VueSelect
		...
		<template v-if="helperText || $scopedSlots.footer" #footer="data">
			<p
				v-if="helperText"
		...
		<template v-for="name in forwardedSlots" #[name]="data">
...
export default {
	...
	computed: {
		 ...
		 */
		forwardedSlots() {
			return Object.keys(this.$scopedSlots).filter((name) => name !== 'footer')
		},
...
.v-select.select .select__helper-text {
	display: flex;
	align-items: center;
	margin: 0;
	padding-block: 4px;
