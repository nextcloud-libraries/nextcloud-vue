...
[data-v-new-select].v-select.select {
	...
	&--drop-up {
		&.vs--open {
			// Invert border-radius for drop-up: round bottom, flatten top
			.vs__search .input-field__input {
			...
			// Move floating label to bottom so it doesn't overlap dropdown above
			.vs__search .input-field__label {
				inset-block-start: auto !important;
				inset-block-end: calc(-1.5 * 13px / 2) !important;
			}

			// Multi: move label to bottom (inherits font/color/padding from floated state)
			.select__label {
				top: auto !important;
				bottom: 0 !important;
				transform: translateY(50%) !important;
				// Flip border-radius for bottom position
				border-radius: 0 0 var(--default-grid-baseline) var(--default-grid-baseline) !important;
			}

			// Multi drop-up: invert border only, keep padding unchanged
			// to avoid layout jump (select--drop-up is added after vs--open)
			.vs__dropdown-toggle {
			...
			// Revert the padding from the bottom-open state
			&.vs--multiple .vs__dropdown-toggle {
				padding-block: 0 calc(var(--default-grid-baseline) * 1.5) !important;
			}
		}
	}
	...
	&.vs--single {
		...
		.vs__selected {
			...
			align-items: center;
			padding-block: 0;
			padding-inline: var(--border-radius-element) var(--input-padding-end, calc(var(--default-clickable-area) + var(--default-grid-baseline)));
