...
Pass `profile` to skip the profile request. User status is always fetched by the card.
...
const props = withDefaults(defineProps<{
	 ...
	 * Does not include user status; the card loads status on its own.
	 */
...
async function ensureUserStatus(userId: string) {
	if (!userStatus.value.icon && !userStatus.value.message && !userStatus.value.status) {
